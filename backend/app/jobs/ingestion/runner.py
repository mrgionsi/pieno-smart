...
def run_ingestion(
    *,
    ...
    if download_current:
        ...
        effective_source_name = "mimit.current-page"
    else:
        if not stations_file and not prices_file:
            raise SystemExit(
                "Provide --download-current or at least one of --stations-file / --prices-file."
            )
        if stations_file:
            _log("Parsing station CSV from file", path=stations_file)
            station_data = parse_station_csv(Path(stations_file).read_text(encoding="utf-8"))
            _log(
                "Station CSV parsed",
                extraction_date=station_data.extraction_date.isoformat(),
                rows=len(station_data.rows),
        ...
        if prices_file:
            _log("Parsing price CSV from file", path=prices_file)
            price_data = parse_price_csv(Path(prices_file).read_text(encoding="utf-8"))
            _log(
                "Price CSV parsed",
                extraction_date=price_data.extraction_date.isoformat(),
                rows=len(price_data.rows),
    ...
    _log("Starting database ingestion", source_name=effective_source_name)
    db = SessionLocal()
    try:
